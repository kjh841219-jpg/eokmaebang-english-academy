  ...
  return students.map((student, index) => {
    if (Array.isArray(student)) {
      return {
        ...
        studentPhoneLast4: String(student[3] === "-" ? "" : (student[3] || "")).replace(/[^0-9]/g, "").slice(-4),
    ...
    return {
      ...
      studentPhoneLast4: String(student.studentPhoneLast4 || student.studentPhone || "").replace(/[^0-9]/g, "").slice(-4)
    };
  });
}
...
export async function readPersistentState() {
  ...
  if (process.env.BLOB_READ_WRITE_TOKEN) return await readBlobState();
...
export function persistentStorageStatus() {
  ...
  return {
    ...
    mode: supabase.configured && process.env.BLOB_READ_WRITE_TOKEN ? "supabase-with-blob-fallback" : (supabase.configured ? "supabase" : (process.env.BLOB_READ_WRITE_TOKEN ? "vercel-blob" : "server-memory")),
