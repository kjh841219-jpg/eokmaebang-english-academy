...
function fallbackResult(payload, reason = "") {
  ...
  return {
    ...
    feedback: `사진 속 문제와 학생 답안을 다시 확인해 주세요.${reason ? `\n오류: ${reason}` : ""}`,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      return sendJson(res, 200, fallbackResult(payload, "OPENAI_API_KEY가 설정되어 있지 않습니다."));
    }

    const images = Array.isArray(payload.images) ? payload.images.filter(image => /^data:image\/(png|jpeg|jpg|webp);base64,/i.test(image.dataUrl || "")) : [];
    if (!images.length) {
      return sendJson(res, 400, {ok: false, error: "분석할 숙제 사진이 없습니다."});
    }
    if (images.length > 6) {
      return sendJson(res, 400, {ok: false, error: "사진은 한 번에 최대 6장까지 분석할 수 있습니다."});
    }

    const student = payload.student || {};
    const prompt = `
너는 벌교미래엔영어학원의 숙제 채점 및 학생별 피드백 담당 AI 선생님이다.
업로드된 숙제 사진을 보고 문제, 학생 답안, 예상 정답을 최대한 정확히 판독해라.

[중요 원칙]
1. 정답지가 없어도 사진 속 문제를 직접 풀어 예상 정답을 도출한다.
2. 학생 답안이 흐리거나 가려져 있으면 절대 단정하지 말고 "추가 확인 필요"라고 표시한다.
3. 영어 문법, 단어, 리딩, 영작 문제는 원어민 교사 수준으로 문법 오류 없이 판단한다.
4. 학생에게 보여줄 피드백은 혼내는 말투가 아니라 무엇을 고치면 좋아지는지 분명하게 쓴다.
5. 학부모에게 보낼 피드백은 따뜻하지만 구체적으로, 모바일 카카오톡에서 읽기 좋게 작성한다.
...
반드시 JSON 객체만 반환하라. 마크다운 설명은 금지한다.
{
  ...
  "needs_review": true 또는 false
    ...
    if (!aiRes.ok) {
      return sendJson(res, 200, fallbackResult(payload, data.error?.message || "OpenAI API 호출 실패"));
    }
    ...
    return sendJson(res, 200, fallbackResult(payload, error.message || "숙제 사진 분석에 실패했습니다."));
  }
}
