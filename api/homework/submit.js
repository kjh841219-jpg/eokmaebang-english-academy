...
function normalizeRecord(result, payload, student) {
  ...
  return {
    ...
    submittedFrom: "student-homework-submit"
  };
}
  ...
  try {
    ...
    if (!matches.length) {
      return sendJson(res, 404, {
        ok: false,
        error: `뒷자리 ${last4}와 일치하는 학생을 찾지 못했습니다. 학원에 등록된 학생 또는 보호자 연락처를 확인해 주세요.`
      });
    }
    ...
    if (matches.length > 1 && !selectedStudentId) {
      ...
      return sendJson(res, 200, {ok: true, needsSelection: true, needSelect: true, choices, result: {choices}});
    }
    ...
    if (!imageCount) {
      return sendJson(res, 400, {ok: false, error: "제출할 숙제 사진을 업로드해 주세요."});
    }
    ...
    return sendJson(res, 500, {ok: false, error: error.message || "숙제 제출에 실패했습니다."});
  }
}
